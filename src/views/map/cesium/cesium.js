...
// 初始化
export function initCesium(cesiumObjs, CesiumNavigation) {
  cesiumObjs.viewer = initViewer(Cesium)
  initNavigation(CesiumNavigation, cesiumObjs.viewer)
  getPositionByMouse(cesiumObjs)
}
...
function initBaseLayer(viewer) {
  var img_arcgis_yxdt = new Cesium.ProviderViewModel({
    name: 'ESRI影像',
    tooltip: 'ESRI影像',
    iconUrl: 'https://cesiumjs.org/releases/1.56.1/Build/Cesium/Widgets/Images/ImageryProviders/esriWorldImagery.png',
  ...
  var img_tdt_yx = new Cesium.ProviderViewModel({
    ...
    creationFunction: function() {
      var tian1 = new Cesium.WebMapTileServiceImageryProvider({
        url: 'http://t0.tianditu.com/img_w/wmts?service=wmts&request=GetTile&version=1.0.0&LAYER=img&tileMatrixSet=w&TileMatrix={TileMatrix}&TileRow={TileRow}&TileCol={TileCol}&style=default&format=tiles&tk=d4747908a3f86c33dcfa0d02d74106a6',
        layer: 'tdtBasicLayer',
        style: 'default',
        format: 'image/jpeg',
      ...
      var zj = new Cesium.WebMapTileServiceImageryProvider({
        url: 'http://t0.tianditu.com/cia_w/wmts?service=wmts&request=GetTile&version=1.0.0&LAYER=cia&tileMatrixSet=w&TileMatrix={TileMatrix}&TileRow={TileRow}&TileCol={TileCol}&style=default.jpg&tk=d4747908a3f86c33dcfa0d02d74106a6',
...
// 鼠标位置追踪
function getPositionByMouse(cesiumObjs) {
  // 得到当前三维场景
  ...
  var handler = new Cesium.ScreenSpaceEventHandler(
    cesiumObjs.viewer.scene.canvas
  )
  // var ray;
  handler.setInputAction(event => {
    ...
    cartesian = cesiumObjs.viewer.scene.pickPosition(event.endPosition)
    if (cartesian) {
      // 将笛卡尔坐标转换为地理坐标
      ...
      // 获取相机高度
      height = Math.ceil(
        cesiumObjs.viewer.scene.camera.positionCartographic.height
      )
      // debugger;
